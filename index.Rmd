...
```
One unique State ID, Puerto Rico, was lost when the datasets were joined, making the distinct number of unique states 52. This was done because the healthinsurance dataset did not include this state, so including it would have left NA values in the dataset that would have prevented evaluation of this state. 
...
In this section, I calculated the mean, standard deviation, maximum, and minimum summary statistics for the three numeric variables within my joined dataset (UninsuredRatePerc, MedicaidEnrollment, and DeathRate).
```{R}
library(knitr)
...
```
The first table shows statistics for UninsuredRateperc, with the initial table being filtered to show statistics for the United States as a whole. The second table shows the same statistics for each state.  
...
```
These tables show the same statistics for the U.S. at large and the states together for Medicaid Enrollment before and after the establishment of the ACA. 
```{R}
...
```
These tables show the the summary statistics for DeathRate. The first table includes all cancer type and sex data, while the second table shows the statistics by specific cancer type. 

```{R}
fulldata %>% count(State)
fulldata %>% count(UninsuredYear)
fulldata %>% count(MedicaidYear)
fulldata %>% count(CancerType)
fulldata %>% count(MedicaidExpansion)
fulldata %>% summarize_all(funs(sum(is.na(.))))
```
...
nnfulldata %>% group_by(State, col1, col2, UninsuredChange) %>% summarize(PercentageDueToType = changeinunins(UninsuredChange)) %>% mutate(PercentageDueToType) %>% arrange(desc(PercentageDueToType)) %>% distinct(PercentageDueToType)


fulldata %>% distinct(CancerType) %>% filter(str_detect(CancerType, "Male")) %>% count()


fulldata %>% group_by(UninsuredYear, MedicaidExpansion) %>% summarize(n_distinct(UninsuredRateperc))
```
In this section, I explored the dataset by grouping and filtering to reduce the data in order to display any potential relationships. First, I identified the number of distinct cancer types (excluding those that counted all types), and then I created a function to divide a variable by this number. I then took the Uninsured Rates for the years 2010 and 2015 and calculated the change in those rates for each state, and plugged that into the function to theoretically account for how much each cancer type affects the change in Uninsured. Though this is only conceptual, it's interesting to imagine how much of a role each cancer type would play in this change between states, and this goes to demonstrate the influence of greater insurance access and greater use of medical assistance. Next, I used str_detect to find the number of cancer types that collected statistics for males, and lastly, I grouped by two categorical variables to find the distinct number of Uninsured based on UninsuredYear and MedicaidExpansion. 
